...
ENV VARS required: POLYGON_KEY, FINTEL_KEY, TG_TOKEN, TG_CHAT
"""
import os, time, requests, datetime, logging
from math import isclose

TICKERS = [
    'RGC','LUCY','PONY','BDMD','MRNO','TIVC',
...
def quote(tic):
    j=requests.get(f'https://api.polygon.io/v1/last/stocks/{tic}?apiKey={POLY}',
                   headers=HEADERS,timeout=10).json()['last']
...
def rvol(tic,today_vol):
    ...
    avg=sum(d['v'] for d in data)/len(data); return today_vol/avg if avg else 0
    ...
    while True:
        for tic in TICKERS:
            try:
                ...
                if fee<100 or avail>2000: continue
                _rvol       = rvol(tic,vol)
                _pdh        = pdh(tic)
                vw          =
